...
The model is H = -1/2 d²/dx² + x²/2 with ℏ = m = ω = 1. The potential MPO is
fitted directly from analytic Fourier coefficients by TCI, the derivative is
an exact Fourier-space MPO, and the ground state is obtained with ITensor DMRG.
"""
...
function build_sho(config::SHOConfig)
    ...
    # V(x) = x²/2 is fitted directly as paired output/input TCI sites.
...
function print_report(io::IO, config, model, energy, state, dmrg_diagnostics,
    ...
    @printf(io, "1D harmonic oscillator: H = -1/2 d²/dx² + x²/2\n")
    @printf(io, "qubits                  = %d\n", config.qubits)
...
function main(config::SHOConfig=SHOConfig())
    ...
    plot_realspace(
        ...
        quantity=:real,
        title="1D SHO ground state from Fourier-QTT DMRG",
    )
    plot_realspace(
        comparison.x,
        comparison.error;
