...
potential MPO is fitted independently from analytic Fourier coefficients by
TCI, each dimension's kinetic term is an exact Fourier-space MPO, the two
...
function print_report(io::IO, config, model, energy, state, dmrg_diagnostics,
    ...
    @printf(io, "2D harmonic oscillator: H = -1/2(d²/dx²+d²/dy²) + (x²+y²)/2\n")
